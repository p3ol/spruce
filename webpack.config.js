...
module.exports = ({ service, spruceConfig = {} } = {}) => {
  ...
  let baseConfig = {
          ...
          options: {
            presets: [
              [require.resolve('@babel/preset-env'), {
                targets: isCoreServer
                  ? { node: 'current' } : ['last 2 version'],
              }],
    ...
    resolve: {
      alias: {
        '@poool/spruce-config': path.resolve('spruce.config.js'),
      },
    },
    plugins: [
      ...(isCoreServer ? [
        new webpack.DefinePlugin({
          'global.spruceConfig': spruceConfig,
        }),
        // new CopyPlugin([
